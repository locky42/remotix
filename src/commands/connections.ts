...
export function registerConnectionCommands(saveConnection: Function) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('tentacle.export', async () => {
    try {
      const config = ConfigService.getGlobalConfig();
      ...
      const homeDir = process.env.HOME || process.env.USERPROFILE || '';
      const defaultUri = vscode.Uri.file(path.join(homeDir, 'tentacle-export.json'));
  ...
  context.subscriptions.push(vscode.commands.registerCommand('tentacle.import', async () => {
    try {
      const homeDir = process.env.HOME || process.env.USERPROFILE || '.';
      const uris = await vscode.window.showOpenDialog({
        canSelectFiles: true,
